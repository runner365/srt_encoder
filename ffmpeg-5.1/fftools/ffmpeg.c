...
void congestion_ctrl(AVFormatContext *s);
void srt_log(const char* fmt, ...);
extern int update_srt_enable(OutputFile* of);
...
static URLContext* srt_ffio_geturlcontext(AVIOContext *s) {
    ...
    internal = (URLContext*)s->opaque;

    return (void*)(internal->priv_data);
}
...
static void reset_vencode_bitrate(void) {
    ...
    if ((now_ts - s_last_video_perf_ts) > RESET_INTERVAL) {
        if (s_congestion_state == STATE_INCR) {//not adjust bitrate for bandwith is enough
            ...
            if (s_current_bitrate < s_video_bitrate*1.5) {
            ...
            s_current_bitrate = s_enc_ctx->bit_rate;
            snprintf(oper_sz, sizeof(oper_sz), "keep");
            snprintf(debug_buffer, sizeof(debug_buffer), 
                "{\"timestamp\":\"%ld\", \"oper\":\"%s\", \"minrtt\":\"%d\", \"real_bitrate\":\"%ld\", \"conf_bitrate\":\"%ld\"}",
                now_ts, oper_sz, s_rtt_min, s_current_bitrate, s_video_bitrate);
...
void congestion_ctrl(AVFormatContext *s) {
    ...
    av_log(s, AV_LOG_DEBUG, "congestion ctrl srtctx(%p:%d) return %d, rtt:%d, inflight:%ld, bw_bitrate:%ld, url:%s\r\n", 
        srtctx, srtctx->fd, ret, rtt, inflight, bw_bitrate, s->url);
    if (ret != 0) {
        return;
    }
                              ...
                              char *error, int error_len)
{
    ...
    if (ost->enc_ctx) {
        ...
        if ((ost->enc_ctx->codec_type == AVMEDIA_TYPE_VIDEO) && ((ost->enc_ctx->codec_id == AV_CODEC_ID_H264)
    ...
    if (ret < 0)
        return ret;
    (void)update_srt_enable(output_files[ost->file_index]);
    return ret;
}
